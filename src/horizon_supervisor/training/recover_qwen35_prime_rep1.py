...
EXPECTED_CONTRACT_SHA256 = (
    "4496ff86ef6d2d6f8a8adbe1bea631e56132ad63847475a0040b5cfc0498e129"
)
SOURCE_LABEL = "small-all-rep1"
RECOVERY_LABEL = "recovery-qwen35-9b-prime-rep1"
EXPECTED_KEY = ("python-prime-http-server", "swiss/fixed-qwen35-9b", 1)
...
def consolidate_recovery_rows(
    ...
    *,
    ...
    if len(source_rows) != EXPECTED_SOURCE_ROWS:
        raise ValueError("source replication must contain exactly 10 outcomes")
    ...
    if _row_key(invalid_row) != EXPECTED_KEY:
        raise ValueError("recovery target is not the frozen Qwen3.5-9B prime pair")
    ...
    if len(keys) != len(set(keys)) or len(consolidated) != EXPECTED_SOURCE_ROWS:
        raise ValueError("recovered replication is not a unique 10-task route matrix")
    if any(
...
def _summary(
    ...
    *,
    ...
    return {
        ...
        "expected_record_count": EXPECTED_SOURCE_ROWS,
...
def _existing_learning_valid_recovery(
    ...
    report_path = result_path.parents[3] / "report.json"
...
def run_recovery(root: Path, manifest_path: Path, contract_path: Path) -> dict[str, Any]:
    ...
    design = contract["replication_design"]
    if not design.get("infrastructure_recovery_only"):
        raise RuntimeError("contract is not restricted to infrastructure recovery")
    if design["run_label"] != RECOVERY_LABEL:
    ...
    replaces_outcome_id = str(design["replaces_outcome_id"])
    invalid_source = next(
        row for row in source_rows if row["outcome_id"] == replaces_outcome_id
    )
    consolidate_recovery_rows(
        ...
        [
            {
                **invalid_source,
                "outcome": {**invalid_source["outcome"], "status": "verified"},
            }
        ],
    ...
    if backup_path.exists():
        raise RuntimeError("pre-recovery backup unexpectedly already exists")
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Recover the frozen Qwen3.5-9B prime replication-1 pair"
    )
    ...
    parser.add_argument(
        ...
        default=Path(
            ...
            "recovery-qwen35-9b-prime-rep1.json"
        ),
    )

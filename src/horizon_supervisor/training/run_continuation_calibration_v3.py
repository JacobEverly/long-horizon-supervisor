...
from horizon_supervisor.training.run_continuation_calibration_v1 import (
    _continuation_command as _v1_continuation_command,
)
from horizon_supervisor.training.run_continuation_calibration_v1 import (
...
STATE_PATH = OUTPUT_ROOT / "execution-state-v3.json"
OUTCOMES_PATH = OUTPUT_ROOT / "natural-continuation-outcomes-v3.jsonl"
USAGE_PATH = OUTPUT_ROOT / "trial-usage-ledger-v3.jsonl"
CHECKPOINT_INDEX = OUTPUT_ROOT / "checkpoint-bank-index-v3.jsonl"
FIDELITY_REPORT = OUTPUT_ROOT / "snapshot-fidelity-v3.json"
LEDGER_PATH = OUTPUT_ROOT / "execution-ledger-v3.json"
EXPECTED_SCHEMA = "two-tier-continuation-calibration-manifest.v3"
...
def validate_manifest(
    ...
    if not sidecar.is_file():
        raise RuntimeError("frozen v3 manifest sidecar is missing")
    expected_hash = sidecar.read_text(encoding="utf-8").split()[0]
    if _sha256(manifest_path) != expected_hash:
        raise RuntimeError("frozen continuation v3 manifest hash mismatch")
    if manifest.get("schema_version") != EXPECTED_SCHEMA:
        raise RuntimeError("unexpected continuation v3 manifest schema")
    if manifest.get("frozen_before_model_outcomes") is not True:
        raise RuntimeError("v3 manifest was not frozen before outcomes")
    lineage = manifest["lineage"]
    if (
        lineage["v2_manifest_sha256"] != EXPECTED_V2_MANIFEST_SHA256
        or lineage["v2_failure_sha256"] != _sha256(V2_FAILURE)
        or lineage["v2_terminal_outcome_count"] != 0
        or lineage["minimal_revision"]
        != "add only tail to the harness-only process allowlist"
        ...
        raise RuntimeError("v3 lineage or minimal revision changed")
    if manifest["models"]["routes"] != EXACT_MODELS:
        raise RuntimeError("frozen v3 route/model mapping changed")
    if manifest["detector"]["config"] != SELECTED_CONFIG.model_dump():
        raise RuntimeError("frozen v3 detector config changed")
    process = manifest["process_reproducibility"]
    if (
        process["schema_version"] != "action-process-delta.v2"
        or process["frozen_harness_only_additions"] != ["tail"]
    ):
        raise RuntimeError("v3 process exclusion changed")
    ...
    if (
        ...
        raise RuntimeError("frozen v3 execution contract changed")
    budget = manifest["budget"]
    if (
        float(budget["project_openrouter_spend_before_usd"]) != 49.308781292
        or float(budget["project_cumulative_ceiling_usd"]) != 200.0
        or float(budget["phase_a_incremental_ceiling_usd"]) != 4.93
        or float(budget["tranche_1_incremental_ceiling_usd"]) != 1.6
        or float(budget["tranche_2_incremental_ceiling_usd"]) != 3.25
        or float(budget["per_trial_incremental_ceiling_usd"]) != 0.5
    ):
        raise RuntimeError("frozen v3 budget changed")
    tasks = manifest["task_selection"]["ordered_pool"]
    if len(tasks) != 24 or [row["position"] for row in tasks] != list(range(1, 25)):
        raise RuntimeError("frozen v3 task pool changed")
    if [row["tranche"] for row in tasks] != [1] * 8 + [2] * 8 + [3] * 8:
        raise RuntimeError("frozen v3 tranches changed")
    fixed_files = {
    ...
    for task in tasks:
        ...
        if not task_root.is_dir() or _tree_sha256(task_root) != task["task_tree_sha256"]:
            raise RuntimeError(f"frozen v3 task tree changed: {task['task_id']}")
    for path, digest in fixed_files.items():
        if not path.is_file() or _sha256(path) != digest:
            raise RuntimeError(f"frozen v3 input hash mismatch: {path}")
...
def _continuation_command(**kwargs: Any) -> list[str]:
    command = _v1_continuation_command(**kwargs)
    command[command.index("--agent") + 1] = (
        "horizon_supervisor.benchmark.continuation_harbor_v2:"
...
def _run_trial(
    *,
    ...
    if ceiling - usage_before < float(
        ...
        raise RuntimeError("frozen OpenRouter v3 tranche ceiling lacks one reserve")
    _post_json(f"{server.base_url}/v1/stats/reset")
    timestamp = datetime.now(UTC).strftime("%Y%m%dT%H%M%S%fZ")
    label = route_id.rsplit("-", 1)[-1]
    job_name = f"continuation-v3-{task['position']:02d}-{label}-{timestamp}"
...
def _new_state(manifest_hash: str, usage: float, hard_limit: float) -> dict[str, Any]:
    return {
        "schema_version": "continuation-calibration-execution-state.v3",
...
def _persist(state: dict[str, Any]) -> None:
    ...
    _write_json(
        FIDELITY_REPORT,
        {
            "schema_version": "continuation-snapshot-fidelity-report.v3",
...
def run(manifest_path: Path = MANIFEST) -> dict[str, Any]:
    ...
    if initial_sandboxes:
        raise RuntimeError("continuation calibration v3 requires zero sandboxes")
    key_info = query_openrouter_key(api_key)
    if STATE_PATH.exists():
        state = json.loads(STATE_PATH.read_text(encoding="utf-8"))
        if state["manifest_sha256"] != manifest_hash:
            raise RuntimeError("v3 execution state belongs to another manifest")
    else:
        state = _new_state(
        ...
        for tranche in (1, 2, 3):
            for task in manifest["task_selection"]["ordered_pool"]:
                ...
                for route_id, model_id in EXACT_MODELS.items():
                    ...
                    state["pending_trial"] = {
                        "schema_version": "continuation-pending-trial.v3",
            ...
            tranche_report["schema_version"] = (
                "two-tier-continuation-calibration-report.v3"
            )
    ...
    report["schema_version"] = "two-tier-continuation-calibration-report.v3"
    ledger = {
        "schema_version": "continuation-calibration-execution-ledger.v3",
        "status": status,
    ...
    _write_json(OUTPUT_ROOT / "calibration-report-v3.json", report)
    return ledger

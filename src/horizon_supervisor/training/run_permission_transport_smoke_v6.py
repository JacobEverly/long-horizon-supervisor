...
def _settled_cleanup(initial_ids: set[str]) -> tuple[dict[str, Any], list[str]]:
    cleanup = _cleanup_new_sandboxes(initial_ids)
    remaining, final_cleanup_errors = _wait_for_cleanup(initial_ids)
    cleanup_errors = final_cleanup_errors + (cleanup["errors"] if remaining else [])
    return {
        "transient_errors": cleanup["errors"],
        "final_errors": cleanup_errors,
    }, remaining
...
async def _exercise_transport(temporary: Path) -> dict[str, Any]:
    ...
    source = PermissionPreservingDaytonaEnvironment(
        ...
        environment_name="horizon-permission-source-v6",
        session_id="horizon-permission-source-v6",
    ...
    target = SeededDaytonaEnvironment(
        environment_dir=environment_dir,
        environment_name="horizon-permission-target-v6",
        session_id="horizon-permission-target-v6",
...
def run(
    ...
    cleanup, remaining = _settled_cleanup(initial_ids)
    cleanup_errors = cleanup["final_errors"]
    ...
    passed = passed and not cleanup_errors and not remaining and error is None
    report = {
        "schema_version": "permission-transport-smoke.v6",
        "passed": passed,
        ...
        "transient_cleanup_errors": cleanup["transient_errors"],
    ...
    if output_path != ROOT / (
        ...
        raise RuntimeError("v6 smoke output path changed")

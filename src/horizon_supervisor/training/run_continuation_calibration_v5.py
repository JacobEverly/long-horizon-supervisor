...
MANIFEST = OUTPUT_ROOT / "frozen-manifest-v5.json"
STATE_PATH = OUTPUT_ROOT / "execution-state-v5.json"
OUTCOMES_PATH = OUTPUT_ROOT / "natural-continuation-outcomes-v5.jsonl"
USAGE_PATH = OUTPUT_ROOT / "trial-usage-ledger-v5.jsonl"
CHECKPOINT_INDEX = OUTPUT_ROOT / "checkpoint-bank-index-v5.jsonl"
FIDELITY_REPORT = OUTPUT_ROOT / "snapshot-fidelity-v5.json"
LEDGER_PATH = OUTPUT_ROOT / "execution-ledger-v5.json"
REPORT_PATH = OUTPUT_ROOT / "calibration-report-v5.json"
SMOKE_REPORT = OUTPUT_ROOT / "permission-transport-smoke-v5.json"
EXPECTED_SCHEMA = "two-tier-continuation-calibration-manifest.v5"
...
def validate_manifest(
    ...
    if not sidecar.is_file():
        raise RuntimeError("frozen v5 manifest sidecar is missing")
    expected_hash = sidecar.read_text(encoding="utf-8").split()[0]
    if _sha256(manifest_path) != expected_hash:
        raise RuntimeError("frozen continuation v5 manifest hash mismatch")
    if manifest.get("schema_version") != EXPECTED_SCHEMA:
        raise RuntimeError("unexpected continuation v5 manifest schema")
    if manifest.get("frozen_before_model_outcomes") is not True:
        raise RuntimeError("v5 manifest was not frozen before outcomes")
    ...
    if any(lineage.get(key) != value for key, value in expected_lineage.items()):
        raise RuntimeError("v5 lineage hashes changed")
    if (
        ...
        raise RuntimeError("v5 minimal revision contract changed")
    immutable_v4 = {
    ...
    if (
        ...
        raise RuntimeError("frozen v5 execution contract changed")
    expected_smoke_path = (
    ...
    if execution["transport_smoke_path"] != expected_smoke_path:
        raise RuntimeError("v5 transport smoke path changed")
    ...
    for tranche, expected in TRANCHE_DIFFICULTY_COUNTS.items():
        ...
        if actual != Counter(expected):
            raise RuntimeError(f"frozen v5 tranche {tranche} mix changed")
    if any(
        row["static_checkpoint_compatible"] is not True
        or row["prior_terminal_outcome_count"] != 0
        for row in tasks
    ):
        raise RuntimeError("v5 task eligibility changed")

    analysis = manifest["analysis"]
    if (
        analysis["cohort"] != "fresh_v5_only"
        or analysis["aggregate_v3_and_v4"] is not False
        or analysis["prior_outcomes_used_for_fit_or_tuning"] is not False
    ):
        raise RuntimeError("v5 independent analysis contract changed")
    fixed_files = {
    ...
    for path, digest in fixed_files.items():
        if not path.is_file() or _sha256(path) != digest:
            raise RuntimeError(f"frozen v5 input hash mismatch: {path}")
    for task in tasks:
        task_root = ROOT / task["task_root"]
        if not task_root.is_dir() or _tree_sha256(task_root) != task["task_tree_sha256"]:
            raise RuntimeError(f"frozen v5 task tree changed: {task['task_id']}")
...
def validate_transport_smoke(manifest: dict[str, Any]) -> tuple[dict[str, Any], str]:
    ...
    if not path.is_file():
        raise RuntimeError("v5 permission-preserving transport smoke is missing")
    smoke = json.loads(path.read_text(encoding="utf-8"))
    if (
        smoke.get("schema_version") != "permission-transport-smoke.v5"
        ...
        raise RuntimeError("v5 permission-preserving transport smoke did not pass")
...
def _run_trial(
    *,
    ...
    if ceiling - usage_before < float(
        ...
        raise RuntimeError("frozen OpenRouter v5 tranche ceiling lacks one reserve")
    _post_json(f"{server.base_url}/v1/stats/reset")
    timestamp = datetime.now(UTC).strftime("%Y%m%dT%H%M%S%fZ")
    label = route_id.rsplit("-", 1)[-1]
    job_name = f"continuation-v5-{task['position']:02d}-{label}-{timestamp}"
...
def _new_state(
    ...
    return {
        "schema_version": "continuation-calibration-execution-state.v5",
...
def _persist(state: dict[str, Any]) -> None:
    ...
    _write_json(
        FIDELITY_REPORT,
        {
            "schema_version": "continuation-snapshot-fidelity-report.v5",
...
def _report(
    ...
    report["schema_version"] = "two-tier-continuation-calibration-report.v5"
    report["cohort"] = "fresh_v5_only"
    report["new_v5_trajectory_count"] = len(state["outcomes"])
...
def run(manifest_path: Path = MANIFEST) -> dict[str, Any]:
    ...
    if initial_sandboxes:
        raise RuntimeError("continuation calibration v5 requires zero sandboxes")
    key_info = query_openrouter_key(api_key)
    if STATE_PATH.exists():
        state = json.loads(STATE_PATH.read_text(encoding="utf-8"))
        if state["manifest_sha256"] != manifest_hash:
            raise RuntimeError("v5 execution state belongs to another manifest")
        if state["transport_smoke_sha256"] != smoke_hash:
            raise RuntimeError("v5 transport smoke changed after execution began")
    else:
        state = _new_state(
            manifest_hash,
            smoke_hash,
        ...
        for tranche in (1, 2, 3):
            for task in manifest["task_selection"]["ordered_pool"]:
                ...
                for route_id, model_id in EXACT_MODELS.items():
                    ...
                    state["pending_trial"] = {
                        "schema_version": "continuation-pending-trial.v5",
    ...
    ledger = {
        "schema_version": "continuation-calibration-execution-ledger.v5",
        "status": status,
        ...
        "new_v5_trajectory_count": len(state["outcomes"]),
        "analysis_cohort": "fresh_v5_only",
        "prior_outcome_count_used": 0,
        "new_v5_checkpoint_count": sum(

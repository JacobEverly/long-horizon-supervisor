...
from horizon_supervisor.training import (
    amend_stuck_confirmatory_healthy_window as module,
)
...
def test_healthy_window_amendment_preserves_sealed_evidence(
    monkeypatch, tmp_path
) -> None:
    root = tmp_path
    output = root / "artifacts/official/stuck-confirmatory-v1"
    output.mkdir(parents=True)
    source_manifest = output / "frozen-manifest-v2.json"
    amended_manifest = output / "frozen-manifest-v3.json"
    ...
    source = {
        "schema_version": "stuck-confirmatory-manifest.v2",
    ...
    assert amended["schema_version"] == "stuck-confirmatory-manifest.v3"
    amendment = amended["healthy_window_efficiency_amendment"]
    assert amendment["scope"] == "orchestration_only"
    assert amendment["accepted_outcomes_before_amendment"] == 4
    assert "checkpoint timing and eligibility" in amendment["unchanged"]
